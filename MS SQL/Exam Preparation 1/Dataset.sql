...
GO
 ...
 --01. DDL
	...
	--02.Insert
	-- Добавяне на данни в таблицата Tourists
...
-- Добавяне на данни в таблицата Bookings
...
--03.Update
-- Обновяване на датата на заминаване с един ден по-късно за резервации, пристигащи през декември 2023 г.
UPDATE Bookings
SET DepartureDate = DATEADD(day, 1, DepartureDate)
WHERE ArrivalDate >= '2023-12-01' AND ArrivalDate < '2024-01-01';

-- Обновяване на имейл адресите на туристите, чиито имена съдържат "MA", като ги задава на NULL
UPDATE Tourists
SET Email = NULL
WHERE Name LIKE '%MA%';

--04. Delete
-- Изтриване на свързаните записи от таблицата `Bookings`
...
-- Изтриване на туристите с фамилно име "Smith" от таблицата `Tourists`
...
JOIN 
    Countries AS c ON d.CountryId = c.Id
WHERE 
